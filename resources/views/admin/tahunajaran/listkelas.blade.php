...
<div class="col-md-12">
    <div class="card">
      ...
      @if (Auth::user()->role == 'Admin')
        @if ($tahunajaran==$tahunajaran_latest->nama)
                ...
                <tr>
                    ...
                    <td>
                      <form method="post" action="{{ route('kelas.destroy', $data->id) }}">
                            ...
                            <a href="{{ route('datasiswa.kelas.tahunajaran',['id' => $data->nama_kelas, 'angkatan' => $tingkat,'tahunajaran' => $data->tahun_ajaran])}}" class="btn btn-info btn-sm"><i class="fnav-icon fas fa-users"></i> &nbsp; View Siswa</a>

                            @if (Auth::user()->role == 'Admin')
                               @if ($tahunajaran==$tahunajaran_latest->nama)
                                <button type="button" class="btn btn-info btn-sm" onclick="getSubsJadwal('{{$data->nama_kelas}}','{{$data->tahun_ajaran}}')" data-toggle="modal" data-target=".view-jadwal">
  ...
  <script>
    function getCreateKelas(){
      ...
      $('#form_nama').html(`
        ...
        @if($tingkat=='10')
            <select id="nama_kelas" name="nama_kelas" class="select2bs4 form-control @error('nama_kelas') is-invalid @enderror">
      ...
      $('#form_angkatan').html(`
        ...
        <input type='hidden' id="angkatan" value="{{$tingkat}}" name='angkatan'>
      `);
       $('#form_tahunajaran').html(`
        <input type='hidden' id="tahun_ajaran" value="{{$tahunajaran}}" name='tahun_ajaran' class="form-control @error('tahun_ajaran') is-invalid @enderror" placeholder="{{ __('Nama Kelas') }}">
      `);
      $('#nama_kelas').val('');
      $('#form_paket').html('');
      $('#form_paket').html(`
        <label for="paket_id">Penjurusan</label>
    ...
    function getSubsSiswa(id){
      ...
      $("#link-siswa").attr("href", "https://sibime/listsiswapdf/"+id);
    }
    
    function getSubsJadwal(id,tahun){
      var parent = id;
      var tahunajaran=tahun;
      $.ajax({
        type:"GET",
        data: { nama_kelas: parent, tahun_ajaran: tahunajaran},
        dataType:"JSON",
        ...
        error:function(){
            $("#judul-jadwal").text('View Data Jadwal '+parent);
             $("#data-jadwal").html('');
          },
        complete:function(){
        }
      });
      $("#link-jadwal").attr("href", "https://siakad.didev.id/"+id);
    }
